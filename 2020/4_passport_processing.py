...
INPUT = """byr:1985
eyr:2021 iyr:2011 hgt:175cm pid:0208311541 hcl:#18171d
...
INPUT_DEBUG = """hgt:168cm hcl:#866857 iyr:2012 pid:1527348755
byr:1946 eyr:2028 cid:184 ecl:amb
"""
...
def is_between(input, x, y):
    if input.isdigit() and int(input) >= x and int(input) <= y:
        return True
    return False
...
def is_valid(passport):
    for field in REQUIRED_FIELDS:
        ...
        if field == "byr":
            if not is_between(value, 1920, 2002):
                return False
        elif field == "iyr":
            if not is_between(value, 2010, 2020):
                return False
        elif field == "eyr":
            if not is_between(value, 2020, 2030):
                return False
        elif field == "hgt":
            index_cm = value.find("cm")
            index_in = value.find("in")
            if index_cm > 0:
                if not is_between(value[0:index_cm], 150, 193):
                    return False
            elif index_in > 0:
                if not is_between(value[0:index_in], 59, 76):
                    return False
            else:
                return False
        elif field == "hcl":
            if not re.search(r'^#(?:[0-9a-f]{3}){1,2}$', value):
                return False
        elif field == "ecl":
...
if __name__=="__main__":
    ...
    for line in lines:
        ...
        for field in fields:
            entries = field.split(":")
            current_passport[entries[0]] = entries[1]
