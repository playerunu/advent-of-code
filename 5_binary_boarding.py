...
def extract_coordinates(seat_id):
    return divmod(seat_id, 8) 
...
if __name__=="__main__":
    ...
    for line in lines:
        seat_id = decode_boarding_pass(line)
        if seat_id > max:
            max = seat_id

    for i in range(128):
        for j in range(8):
            if i > 0 and i < 127 and j > 0 and j < 7 and seats[i][j] == 0:
                my_id = calculate_seat_id(i, j)
                
                # Verify seat_id + 1
                (x, y) = extract_coordinates(my_id + 1)
                if seats[x][y] == 0:
                    continue
