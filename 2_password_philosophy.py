...
if __name__=="__main__":
    valid_passwords = 0
    valid_password_policy2 = 0
    
    lines = INPUT.splitlines()
    for line in lines:
        tokens = line.split(" ")
        
        # Extract lowest and highest number of times 
        low = int(tokens[0].split("-")[0])
        high = int(tokens[0].split("-")[1])
        ...
        exists_on_positions = (password[low-1] == letter) + (password[high-1] == letter)
        if (exists_on_positions == 1):
            valid_password_policy2 += 1
    
    print(valid_passwords)
    print(valid_password_policy2)
